...
if ~isempty(userClickData) %Make sure there is data
    ...
    for u = 1:size(selectedUsers,1) %Go through each user
        ...
        for r = 1:size(subRoutes,2) %Go through each route for that sub-app
            ...
            outputClick.route(u,r) = subRoutes(r);
            %Total clicks used in sub-app by user
            outputClick.totalClicksSum(u,r) = sum(indRoute);         
            %Calculate Average Click Per Page Visit
            indClicksBlock = find(indRoute == 1); %find index of all routes 
            outputClick.visits(u,r) = sum(diff(indClicksBlock) ~= 1); %If difference is bigger than 1 it means the clicks did not occur at the same time. Segment clicks by blocks. 
            outputClick.avgClickPerPage(u,r) = sum(indRoute)/outputClick.visits(u,r);
